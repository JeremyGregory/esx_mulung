...
Citizen.CreateThread(function()
    ...
    for locationIndex = 1, #Config.SellBottleLocations do
        ...
        AddTextComponentString("Jual Botol")
    ...
    while true do
        ...
        for locationIndex = 1, #Config.SellBottleLocations do
            ...
            if dstCheck <= 5.0 then
                ...
                local text = "Jual Botol"

                if dstCheck <= 1.5 then
                    text = "[~g~E~s~] " .. text

                    if IsControlJustReleased(0, 38) then
                        TriggerServerEvent("wr-mulung:sellBottles")
                    end
                end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if DoesEntityExist(entity) and entityDst <= 1.5 then
            ...
            ESX.Game.Utils.DrawText3D(binCoords + vector3(0.0, 0.0, 0.5), "[~g~E~s~] Mulung", 0.4)
            ...
            if IsControlJustReleased(0, 38) then
                if not cachedBins[entity] then
                    ...
                    exports['mythic_notify']:SendAlert('error', 'Anda sudah mencari tempat sampah ini')
                end
            end
        end
...
function OpenTrashCan()
    ...
    TriggerServerEvent("wr-mulung:retrieveBottle")
